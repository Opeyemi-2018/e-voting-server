  ...
  try {
    ...
    if (!uniqueNumberDoc) {
      return res.status(404).json({
        msg:
          "Invalid unique number. Please provide a valid unique number to vote.",
      });
    }
    ...
    if (uniqueNumberDoc.used) {
      return res
        .status(400)
        .json({ msg: "This unique number has already been used to vote." });
    }
    ...
    for (let vote of votes) {
      ...
      if (existingVote) {
        return res
          .status(400)
          .json({ msg: `You have already voted in the ${category} category.` });
      }
